...
import os
conn = sqlite3.connect('C:\\Working_Folder\\EEC\\Project\\Python\\warehouse.db')

# Menu for sqlite3 Warhouse Management project
def Warehouse_menu():
    while True:
        print('''
              --------------------------
        ...
        if W_option == 1:
            ...
            print('\n')
            c = conn.cursor()
            c.execute("SELECT * FROM Warehouse WHERE upper(warehouse_city)= ?",(WCity,))
            results =tabulate(c.fetchall(),headers=["warehouse_id", "warehouse_address", "warehouse_email", "warehouse_phone", "warehouse_city", "warehouse_state"])
            if results:
                print(results)
                print('\n')
            conn.commit()
...
def Logistics_menu():
    while True:
        ...
        if L_option == 1:
            ...
            L_phone = input("Enter Logistics provider's phone number: ")
            c = conn.cursor()
            c.execute('''UPDATE Logistics_Provider SET provider_address=?, provider_phone=?, provider_email=? WHERE provider_id =?''', (L_address, L_phone, L_email, L_identity))
            conn.commit()
        elif L_option == 0:
            MainMenu()
        else:
...
def Customer_Order():
    while True:
        print('''
              ...
              5. Order Invoice 
              6. Order pending list
              7. Shipment status search / update
              0. Previous menu
              ''')
        ...
        if C_order_option == 1:
            ...
            print('\n')
            c = conn.cursor()
            c.execute('''SELECT order_id, shipment_status, warehouse_id, tracking_reference FROM Customer_Shipment LIMIT 5''')
            ...
            print("------------------------------------------------------------------------------------------------")
            c.execute('''SELECT sub_order_id, product_name, COD.quantity, COD.sell_price, COD.tax_rate, (COD.quantity*COD.sell_price*(1+COD.tax_rate)) subtotal FROM Customer_Order_details COD, Product_Inventory PI WHERE COD.product_id= PI.product_id AND order_id=?''',(t_order_id,))
            results =tabulate(c.fetchall(),headers=["sub_order_id", "product_name", "quantity", "sell_price", "tax_rate", "sub_total"])
            if results:
                print(results)
            print('\n')
